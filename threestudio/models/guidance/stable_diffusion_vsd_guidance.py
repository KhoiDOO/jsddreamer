...
class StableDiffusionVSDGuidance(BaseModule):
    ...
    def compute_grad_vsd(
        ...
        if self.cfg.analysis:
            latents_denoised = self.get_x0(latents_noisy, noise_pred_pretrain, t).detach()
            grad_x0 = latents_denoised - latents_noisy
        else:
            grad_x0 = None
    ...
    def forward(
        ...
        **kwargs,
        ...
        grad, grad_img, grad_x0 = self.compute_grad_vsd(latents, text_embeddings_vd, text_embeddings, camera_condition)
        ...
        if grad_x0 is not None:
            loss_dict['grad_x0_norm'] = grad_x0.norm()
